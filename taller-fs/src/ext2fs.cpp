...
struct Ext2FSInode * Ext2FS::load_inode(unsigned int inode_number)
{
	...
	int inodos_por_bloque = block_size/ _superblock->inode_size;
	// numero de block group

	unsigned int block_group_number =  blockgroup_for_inode(inode_number);

	unsigned int lba_inode_table = block_group(block_group_number)->inode_table;

	Ext2FSInode* block_inode_table = (Ext2FSInode*) malloc(block_size);

	unsigned int indice = blockgroup_inode_index(inode_number);

	read_block(lba_inode_table + (indice/inodos_por_bloque),(unsigned char*) block_inode_table );
	
	Ext2FSInode* inodo_res = (Ext2FSInode*) malloc(_superblock->inode_size);
	
	memcpy(inodo_res,&block_inode_table[indice % inodos_por_bloque],_superblock->inode_size);

	free(block_inode_table);
...
unsigned int Ext2FS::get_block_address(struct Ext2FSInode * inode, unsigned int block_number)
{
	...
	int cant_direccions_bloque = block_size/4;
	int limite_directos = 12;
	int limite_indirecto = limite_directos + cant_direccions_bloque;

	if(block_number < limite_directos){
		return inode->block[block_number];
	}
	if(block_number < limite_indirecto){
		//caso indirecto simple
		unsigned int block_number_tmp =  block_number - limite_directos;

		unsigned int* buffer = (unsigned int*) malloc(block_size);

		read_block(inode->block[12],(unsigned char *)buffer);

		unsigned int lba = buffer[block_number_tmp];

		free(buffer);

		return lba;
	}
	//caso indirecto doble
	unsigned int block_number_tmp =  block_number - limite_indirecto;

	unsigned int* buffer_ind = (unsigned int*) malloc(block_size);

	read_block(inode->block[13],(unsigned char*) buffer_ind);

	unsigned int lba_indirecta = buffer_ind[block_number_tmp / cant_direccions_bloque];

	free(buffer_ind);

	unsigned int* buffer_d = (unsigned int*) malloc(block_size);

	read_block(lba_indirecta,(unsigned char *) buffer_d);

	unsigned int lba = buffer_d[block_number_tmp % cant_direccions_bloque];
...
struct Ext2FSInode * Ext2FS::get_file_inode_from_dir_inode(struct Ext2FSInode * from, const char * filename)
{
	...
	unsigned int block_size = 1024 << _superblock->log_block_size;
	unsigned int bytes_actuales = 0;
	
	unsigned char* buffer_dir = (unsigned char*) malloc(2*block_size);
	unsigned int numero_de_bloque = 0;

	unsigned int bloque = get_block_address(from,numero_de_bloque);
	read_block(bloque, buffer_dir);
	numero_de_bloque++;
	bloque = get_block_address(from,numero_de_bloque);
	read_block(bloque,buffer_dir + block_size);
	numero_de_bloque++;
	unsigned char* offset = buffer_dir;

	while (bytes_actuales < from->size){
		
		while( offset < (buffer_dir + block_size) ){
				
			Ext2FSDirEntry* dir = (Ext2FSDirEntry*)	offset;				
			if((strncmp(dir->name,filename,dir->name_length) == 0 && dir->name_length==strlen(filename))){
				unsigned int inode_number = dir->inode;
				Ext2FSInode* inode_res = load_inode(inode_number);

				free(buffer_dir);
				return inode_res;
			 }
			
			 bytes_actuales += dir->record_length;	 
			 offset = offset +  dir->record_length;	 

		}
	
		offset -= block_size;
		
		memcpy(buffer_dir,buffer_dir + block_size,block_size);

		if(numero_de_bloque * block_size < from->size){
			unsigned int bloque = get_block_address(from,numero_de_bloque);
			read_block(bloque,buffer_dir + block_size);
			numero_de_bloque++;
		}
